...
void main(void)
{
	...
	while (true)
	{
		...
		case MATRICULAR_ALUNO:
			...
			if (BuscarPeriodo(&nPeriodo, QTDE_PERIODOS, "Consulta de Período") == true)
			{
			...
			if (PedirPeriodoRaia(&nPeriodo, &nRaia,
				QTDE_PERIODOS, QTDE_RAIAS_DA_PISCINA, "Confirmação de Matrícula")== true)
			{
				...
				{
					cout << "Esta raia já está ocupada"<< endl;
					PAUSA;
				}
			};
		...
		case CANCELAR_MATRICULA:
			...
			PedirPeriodoRaia(&nPeriodo, &nRaia, QTDE_PERIODOS, QTDE_RAIAS_DA_PISCINA, "Cancelamento de Matrícula");
			if (vetRaiasLivres[nRaia] == false)
			{
				cout << "Esta Raia está vazia." << endl;
				PAUSA;
			}
			else
			{
...
int BuscarRaiaLivreNoPeriodo(bool *ptrVetLivreOcup, int nPeriodo, int nQtRaiasPiscina)
{
	...
	for (i = (nPeriodo - 1)*nQtRaiasPiscina; i<(nPeriodo*nQtRaiasPiscina); i++)
	{
		...
		{
			cout << "Raia " << (i + 1) - ((nPeriodo - 1)*nQtRaiasPiscina) << " está livre!" << endl;
			flgTroca = true;
			PAUSA;
			break;
		}
	...
	if (flgTroca == false)
	{
		cout << "Todas as raias estão ocupadas no momento." << endl;
		PAUSA;
		return 1;
	}
