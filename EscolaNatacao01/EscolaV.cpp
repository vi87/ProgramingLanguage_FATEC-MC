...
void main(void)
{
	...
	char vetSobrenomes[QTDE_RAIAS_DA_PISCINA * QTDE_PERIODOS][TAMANHO_SOBRENOME],
	  vetAuxSobrenomes[TAMANHO_SOBRENOME];	// vetor de sobrenomes
			...
			case MATRICULAR_ALUNO:
				...
				if (BuscarPeriodo(&nPeriodo, QTDE_PERIODOS, "Escolha um período:") == true)
				{
					...
					else if (nRaia >= 0 && vetRaiasLivres[nRaia] != true)
					{
						...
						strcpy_s(vetAuxSobrenomes[0], vetSobrenomes[nRaia]);
						vetRaiasLivres[nRaia] = true;
						cout << "Nome na raia " << nRaia << " é " << vetSobrenomes[nRaia] << endl;
						PAUSA;
					}
				}
				else
				...
				nValorDesconto = BuscarSobrenome(vetSobrenomes[TAMANHO_SOBRENOME], vetSobrenomes[TAMANHO_SOBRENOME], nRaia);
				if (nValorDesconto > 0)
				{
					vetMensalidades[nRaia] = VALOR_MENSALIDADE * (nValorDesconto / 100);
					cout << "Valor da Mensalidade: R$ " << vetMensalidades[nRaia] << endl;
				}
...
int BuscarSobrenome (char *vetNomes[], char *vetAuxNomes[], int Raia)
{
	int i,
		nDesconto=0;
	do
	{
		if (vetNomes[Raia] == vetAuxNomes[i] && Raia != i && nDesconto < 4)
		{
			nDesconto = nDesconto + 1;
		}
		
	} while (i < QTDE_PERIODOS * QTDE_RAIAS_DA_PISCINA);
	nDesconto = nDesconto * VALOR_DESCONTO_POR_PARENTE;
	return nDesconto;
}
