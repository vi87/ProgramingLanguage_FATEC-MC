...
void main(void)
{
	int  i, j, k,							// indices genéricos
		...
		nQtdeTotalQuartos,					// para receber quantidade total de quartos do hotel.
	...
	char cOpcaoOper,						// local para receber a opção escolhida do menu
		...
		cNomeAluno[] = "Correção Prova";
	double dValor, dTotalDiarias;							// local para receber o valor dos serviços do hotel e para somatória de todas as diárias lançadas
	//
	...
	do
	{
		cout << "Informe quantidade de andares entre " << QTDE_MINIMA_ANDARES << " e " <<
			...
			" ou 0 para cancelar a execução do programa: ";
		cin >> nQtdeAndares;
	...
	nQtdeTotalQuartos = nQtdeAndares * nQtdeQuartosPorAndar;		// tolta de quartos
	// loop para inicializar os vetores
	for (i=1, k=0; k < nQtdeTotalQuartos; i++)					// i = andar e k = indice dos quartos
	{
		for(j=0;j < nQtdeQuartosPorAndar;j++, k++)				// j = quartos por andar
		{
	...
	while(true)								// loop infinito
	{
		...
		switch(cOpcaoOper)					// avaliar a opção que foi escolhida pelo operador
		{
			case FAZER_CHECK_IN:
				...
				if(!PedirAndarQuarto(&nAndarHotel, &nQuartoHotel, nQtdeAndares, nQtdeQuartosPorAndar, "Chek-in de um hóspede"))			// Cancelou transação?
					break;						// cai fora do switch e volta pro menu
				// calcular o indice do quarto
				iIndQuarto = ((nAndarHotel -1) * nQtdeQuartosPorAndar) + (nQuartoHotel - 1);
				// verificar se o quartto está ocupado
				...
				cout << "Nome do hóspde: ";
				cin.getline(vetNomeHospede[iIndQuarto], EXTEN_NOME, '\n');		// aceita nom e sobrenome;
				vetLivreOcupado[iIndQuarto] = true;								// ocupa o quarto
				vetTotalDiarias[iIndQuarto] = 0.0;								// zerar as diárias de um hóspde anterior
				sprintf_s(cWork, sizeof(cWork), "\nAndar:%2d Quarto:%3d Hóspede:%-30s\nDiária:%9.2f",
						nAndarHotel, nQuartoHotel, vetNomeHospede[iIndQuarto],
			...
			case LANCAR_DIARIA:
				...
				sprintf_s(cWork, sizeof(cWork),
					"\nLançar diária em %02d/%02/%04d às %02d:%02d Confirma (S ou N): ",
					stTime.wDay, stTime.wMonth, stTime.wYear,
				...
				{ // vai lançar diárias em todos os quarto ocupados
					...
					{
						if(vetLivreOcupado[i])				// QUarto ocupado? (sem nada é true)
						{
							vetTotalDiarias[i] += vetVlrDiaria[i];		// Mais uma diária no quarto
							dTotalDiarias += vetVlrDiaria[i];			// acumula somatória
						}
					}// for i
				}
			...
			case EXIBIR_QUARTOS_LIVRES_ANDAR:
				...
				// loop para listar os quartos que estão livres
				break;
